...
Future<List<Lecture>> getScheduleFromHtml(http.Response response,
  ...
  for (final element in overlappingClasses) {
    ...
    final String? typeClass = element.querySelector('td[headers=t1]')?.nodes[2].text
        ?.trim().replaceAll(RegExp(r'[()]+'), '');
    final String? textDay = element.querySelector('td[headers=t2]')?.text;
    final int? aDay = document.querySelector('.horario > tbody > tr:first-child')
        ?.children
